...
for i in range(NUM):
    ...
    if os.path.isfile(f):
        ...
        os.system('python ~/CS6250_project/scripts/bg_analyze.py --num_decode %s --sim_delta .2 --prop_delta 6 --total_n %s --mols_path %smols.txt < results.%s > analyze.%s' %(N_DECODE, total_n, DATA_DIR, str(i), str(i)) )
